...
my $insertfile_ = "insert into files
   (lfn,full_host_name,full_file_path,time,size,md5) values (?,?,?,'now',?,?)
   on conflict(lfn,full_host_name.full_file_path)
      full_host_name=EXCLUDE.full_host_name,
      full_file_path=EXCLUDE.full_file_path,
      time=EXCLUDE.time,
      size=EXCLUDE.size,
      md5=EXCLUDE.md5;"
...
my $insertfile    = $dbh->prepare($insertfile_);
